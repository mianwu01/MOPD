#!/bin/bash
# v10c: same as v10b strict ExpC clone, only OPD_LOSS_TYPE changed (reverse_kl → forward_kl).
# Hypothesis: reverse_kl mode-seeking caused entropy collapse on Math-7B's sharp
# distribution. Forward KL is mean-seeking → student spreads prob to cover teacher
# modes, preserving entropy → less consistency degradation late in training.
set -e
...
export WANDB_RUN_GROUP=v10c-forwardkl-math7b
export WANDB_TAGS=opd,single-teacher,math-7b,forward-kl,strict-vs-v10b
...
export OPD_LOSS_TYPE=forward_kl       # ← only diff vs v10b
export OPD_CHUNK_SIZE=256
export RUN_ID=v10c-Math7B-forwardkl
