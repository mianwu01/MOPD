...
def _align_vocab(
    ...
    # Qwen2.5 1.5B/3B (V=151936) vs 7B (V=152064): the extra 128 entries on
    # the larger model are reserved/extra tokens with negligible mass during
    # ordinary chat generation. Truncate both to min(V) for shape-safe KL.
    if teacher_logits.shape[-1] != student_logits.shape[-1]:
        common = min(teacher_logits.shape[-1], student_logits.shape[-1])
        teacher_logits = teacher_logits[..., :common]
        student_logits = student_logits[..., :common]
...
def compute_reverse_kl_loss(
    ...
    """
    teacher_logits, student_logits = _align_vocab(teacher_logits, student_logits)
